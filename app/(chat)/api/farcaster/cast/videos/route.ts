...
export async function GET(request: Request) {
  ...
  const response = await fetch(`${NEYNAR_API_URL}/farcaster/cast/search?q=stream.warpcast.com&priority_mode=true&limit=100&viewer_fid=${(session?.user as any).fid ?? 3}`, {
    method: "GET",
    headers: {
