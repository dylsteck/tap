...
export async function GET(request: Request) {
  ...
  if (!ensName || typeof ensName !== "string") {
    return new Response(JSON.stringify({ message: "ENS name is required and must be a string" }), {
      status: 400,
  ...
  try {
    const profileCacheKey = `icebreaker:ens:${ensName}`;
    ...
    if (cachedProfileData) {
      ...
      const response = await fetch(`${ICEBREAKER_API_URL}/ens/${encodeURIComponent(ensName)}`, {
        method: 'GET',
        headers: {
          'Accept': '*/*',
