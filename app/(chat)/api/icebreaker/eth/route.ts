...
export async function GET(request: Request) {
  ...
  if (!walletAddress || typeof walletAddress !== "string") {
    return new Response(JSON.stringify({ message: "Wallet address is required and must be a string" }), {
      status: 400,
  ...
  try {
    const profileCacheKey = `icebreaker:eth:${walletAddress}`;
    ...
    if (cachedProfileData) {
      ...
      const response = await fetch(`${ICEBREAKER_API_URL}/eth/${encodeURIComponent(walletAddress)}`, {
        method: 'GET',
        headers: {
          'Accept': '*/*',
